...
Usage:
  # Search mode
  linkup-search search "your query" [-n <num_results>] [--json]

  # Fetch mode
  linkup-search fetch <url> [--json]

Examples:
  linkup-search search "python web scraping tutorials" -n 10
  linkup-search search "machine learning 2026" --depth deep --json
  linkup-search fetch "https://example.com/article"
  linkup-search fetch "https://example.com" --output-format markdown
...
def parse_arguments():
    ...
    search_parser.add_argument(
        "--from-date", dest="from_date", help="Filter results from date (ISO 8601: YYYY-MM-DD)"
    )
    search_parser.add_argument(
        "--to-date", dest="to_date", help="Filter results to date (ISO 8601: YYYY-MM-DD)"
    )
    search_parser.add_argument(
        "--include-domains",
    ...
    search_parser.add_argument(
        "--json", action="store_true", help="Output results as raw JSON for coding agents"
    )
    ...
    fetch_parser.add_argument("--json", action="store_true", help="Output result as raw JSON")
...
def linkup_search(
    ...
    try:
        ...
        if json_output:
            output = {"mode": "search", "query": query, "error": True, "error_message": str(e)}
            print(json.dumps(output, indent=2))
        else:
            print(f"Error: {e}")
        sys.exit(1)
...
def linkup_fetch(url, output_format="markdown", render_js=False, json_output=False):
    ...
    params = {"url": url, "outputFormat": output_format}
    ...
    try:
        if json_output:
            output = {
                "mode": "fetch",
                "url": url,
                "parameters": {"output_format": output_format, "render_js": render_js},
                "content": "",
                "error": False,
            }
        else:
        ...
        if json_output:
            output = {"mode": "fetch", "url": url, "error": True, "error_message": str(e)}
            print(json.dumps(output, indent=2))
        else:
            print(f"Error: {e}")
        sys.exit(1)
...
def main():
    """Main entry point for the linkup-search CLI tool."""
