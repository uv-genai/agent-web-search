...
Usage: brave-search <search query> [-n <number_of_results>]
Example: brave-search "python programming tutorials" -n 20
         brave-search "best web scraping libraries" --num-results 15
...
def brave_search(query, num_results, json_output=False):
    ...
    try:
        ...
        if json_output:
            output = {"error": True, "query": query, "message": "Request timed out"}
            print(json.dumps(output, indent=2))
        else:
        ...
        if json_output:
            output = {"error": True, "query": query, "message": str(e)}
            print(json.dumps(output, indent=2))
        else:
            print(f"Error: {e}")
        sys.exit(1)
...
def main():
    """Main entry point for the brave-search CLI tool."""
